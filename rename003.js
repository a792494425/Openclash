...
const regexArray = [
  ...
  // 添加新的倍率匹配模式
  /\d+(\.\d+)?[xX]/, // 匹配 1X, 0.2X, 3X, 2.0X 等
  /[xX]\d+(\.\d+)?/, // 匹配 x5, x0.5, x3 等
  /\d+(\.\d+)?倍率/  // 匹配 0.5倍率 等
];
// prettier-ignore
const valueArray = [
  "2×", "3×", "4×", "5×", "6×", "7×", "8×", "9×", "10×", "20×", "30×", "40×", "50×",
  "IPLC", "IEPL", "Kern", "Edge", "Pro", "Std", "Exp", "Biz", "Fam",
...
const nameblnx = /(高倍|(?!1)(0\.|[2-9])+[xX]|\d+(\.\d+)?倍率|[xX]\d+(\.\d+)?|ˣ[²³⁴⁵¹⁰])/i;
const namebllx = /(0\.[0-9]+[xX]|[xX]0\.[0-9]+|0\.[0-9]+倍率)/i;
const namenx = /(高倍|(?!1)(0\.|[0-9])+[xX]|\d+(\.\d+)?倍率|[xX]\d+(\.\d+)?|ˣ[²³⁴⁵¹⁰])/i;
const keya =
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 正则 匹配倍率
    if (bl && inArg.addRateLabel) {
      const match = e.name.match(/((倍率|X|x|×)\D?((\d{1,3}\.)?\d+)\D?)|((\d{1,3}\.)?\d+)(倍|X|x|×)|[xX](\d{1,3}\.?\d*)/i);
      if (match) {
        const rev = parseFloat(match[0].match(/(\d[\d.]*)/)[0]);
        if (rev !== 1) {
          if (rev < 1 && inArg.addLowRateLabel) {
            e.name = e.name + " 低倍率";
          } else if (rev > 1 && inArg.addHighRateLabel) {
            e.name = e.name + " 高倍率";
          }
        }
      }
